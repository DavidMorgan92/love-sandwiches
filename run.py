...
def validate_data(values):
    ...
    try:
        ...
        length = len(values)
        if length != 6:
            raise ValueError(f"Exactly 6 values required, you provided {length}")
...
def calculate_stock_data(data):
    """
    Calculate the average stock for each tiem type, adding 10%
    """
    ...
    for column in data:
        int_column = [int(x) for x in column]
